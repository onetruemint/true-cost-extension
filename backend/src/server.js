...
app.post("/settings", async (req, res) => {
  try {
    ...
    const { enabled, confirm_before_purchase, return_rate, years, min_price } =
      req.body;
...
app.post("/savings", async (req, res) => {
  try {
    ...
    if (!price || !final_decision) {
      return res
        .status(400)
        .json({ error: "Price and final_decision required" });
    }
...
// Start server
app.listen(PORT, () => {
  console.log(`Savest API running on port ${PORT}`);
});
